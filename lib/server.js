...
if (process.argv.length > 2) {
  ...
  try {
    var text = fs.readFileSync(CONFIG_FILE, 'utf8');
    options = JSON.parse(text);
    console.log('read config from '+CONFIG_FILE);
  }
  catch (err) {
    console.log('Error reading config from '+CONFIG_FILE+': '+err.message);
  }
} else {
  console.log('options: node server.js [config.json]');
}
// static files
